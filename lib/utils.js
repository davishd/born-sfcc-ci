...
function getDw() {
    try {
        ...
        throw new Error(`Please check that a valid dw.json exists in the root directory. Error: ${error.toString()}`);
    }
}
...
function getBundles() {
    try {
        ...
        throw new Error(`Please check that a valid config.json exists in the site_data directory. Error: ${error.toString()}`);
    }
}
